<template>
  <div class="absolute inset-0 flex items-center justify-center" style="z-index:50;">
    ...
    <div class="relative w-full max-w-2xl mx-4 border overflow-hidden animate-fade-in-up" :style="panelStyle">
      ...
      <!-- Column header -->
      <div class="px-6 py-2 flex items-center" :style="{ borderBottom:'1px solid rgba(90,79,64,0.2)' }">
        <div style="font-size:9px; color:var(--vn-muted); font-family:monospace; letter-spacing:0.1em;">--- 对话记录 ---</div>
        <div class="flex-1" />
        <div style="font-size:9px; color:var(--vn-muted); font-family:monospace;">共 {{ dialogueLines.length }} 条</div>
      </div>

      <!-- History list -->
      <div ref="scrollRef" class="px-6 py-4 overflow-y-auto no-scrollbar" style="max-height:calc(85vh - 130px);">
        <div
