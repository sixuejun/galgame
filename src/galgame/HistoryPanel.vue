<template>
  <div class="absolute inset-0 flex items-center justify-center" style="z-index: 50">
    ...
    >
      ...
      <div
        ref="scrollRef"
        class="px-6 py-4 overflow-y-auto no-scrollbar"
      ...
      >
        ...
        >
          <div class="flex items-start gap-3">
            <span
              class="text-right shrink-0 pt-0.5"
              style="font-size: 9px; color: rgba(139, 125, 107, 0.5); font-family: monospace; width: 1.5rem"
            >
              {{ String(index + 1).padStart(3, '0') }}
            </span>
            <div class="flex-1 min-w-0">
              <template v-if="line.speaker">
                <span style="color: var(--rust); font-size: 0.75rem; font-weight: bold; letter-spacing: 0.1em">{{
                  line.speaker
                }}</span>
              ...
              <p
                ...
                style="color: rgba(212, 197, 160, 0.6); padding-left: 2em"
              >
                {{ line.text }}
              </p>
            </div>
        ...
        <div
          ...
          style="color: var(--vn-muted); font-size: 0.75rem"
        >
          暂无对话记录
        </div>
      </div>
